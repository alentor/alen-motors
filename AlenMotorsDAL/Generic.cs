...
namespace AlenMotorsDAL {
    internal static class Generic {
        /// <summary>
        /// Encodes a verify string for email using SHA-1 Encryption algorithm with additional 64base string(Generated from salt) for added security 
        /// </summary>
        /// <param name="email">Provide the password</param>
        /// <returns></returns>
        public static string VerifyEmailHash(string email) {
            // Generate a Base64 string
            string salt = DateTime.Now.ToString(CultureInfo.InvariantCulture);
            byte[] saltBytes = Encoding.UTF8.GetBytes(salt);
            string base64 = Convert.ToBase64String(saltBytes);
            // A small addition to the regular SHA1 encryption for added security
            byte[] passBytes = Encoding.Unicode.GetBytes(email);
